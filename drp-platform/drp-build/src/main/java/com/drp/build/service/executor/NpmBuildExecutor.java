...
public class NpmBuildExecutor implements BuildExecutor {
    ...
    private static final Logger log = LoggerFactory.getLogger(NpmBuildExecutor.class);
    ...
    public NpmBuildExecutor(@Lazy BuildService buildService, ObjectMapper objectMapper) {
    ...
    public BuildResult executeSync(BuildRecord record, Project project, Pipeline pipeline) {
        ...
        try {
            ...
            String npmCommand = extractNpmCommand(pipeline);
            String buildCmd = String.format("%s && npm %s", OsUtil.getCdCommand(workspace), npmCommand);
    ...
    private String extractNpmCommand(Pipeline pipeline) {
        try {
            JsonNode stages = objectMapper.readTree(pipeline.getStages());
            for (JsonNode stage : stages) {
                if ("NPM_BUILD".equals(stage.get("type").asText())) {
