...
@Component
public class PythonBuildExecutor implements BuildExecutor {

    private static final Logger log = LoggerFactory.getLogger(PythonBuildExecutor.class);
    private static final int BUILD_TIMEOUT_SECONDS = 1800;
    private static final String WORKSPACE_BASE = OsUtil.isWindows() ? "D:\\DrpArtifact" : "/tmp/drp-builds";

    private final BuildService buildService;
    private final ObjectMapper objectMapper;

    public PythonBuildExecutor(@Lazy BuildService buildService, ObjectMapper objectMapper) {
    ...
    public BuildResult executeSync(BuildRecord record, Project project, Pipeline pipeline) {
        ...
        try {
            ...
            String pythonCommand = extractPythonCommand(pipeline);
            String buildCmd = String.format("%s && %s", OsUtil.getCdCommand(workspace), pythonCommand);
    ...
    private String extractPythonCommand(Pipeline pipeline) {
        try {
            JsonNode stages = objectMapper.readTree(pipeline.getStages());
            for (JsonNode stage : stages) {
                if ("PYTHON_BUILD".equals(stage.get("type").asText())) {
